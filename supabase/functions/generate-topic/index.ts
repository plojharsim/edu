...
serve(async (req) => {
  ...
  try {
    ...
    Analyzuj veškerý poskytnutý obsah (fotky poznámek, dokumenty, PDF nebo text) a vytvoř z něj ucelenou studijní sadu.
    ...
    const contents = [
      {
        role: "user",
        parts: [
          { text: systemPrompt + `\n\nZadání od uživatele: "${prompt || "Analyzuj přiložené soubory a vytvoř studijní sadu."}"` },
          ...(images || []).map((img: any) => ({
            inline_data: {
    ...
    console.log(`[generate-topic] Requesting Gemini for user: ${user.id} with ${images?.length || 0} files.`);
